...
def proceed(user):
    ...
    with open("orders.txt","r") as f1, open("input.txt","a+") as f2:
         c=f1.read()
         d=c.split("$")
         d[-1]="flag"
         d.remove("flag")
         ...
         str1=b[0]
         if str1[0]=="0":
             if int(str1[1]) in breakfast1:
                 klist=breakfast1
             if int(str1[1]) in lunch1:
                 klist=liunch1
             if int(str1[1]) in dinner1:
                klist=dinner1
         else:
             if int(str1) in breakfast1:
                 klist=breakfast1
             if int(str1) in lunch1:
                 klist=lunch1
             if int(str1) in dinner1:
                klist=dinner1
         b2=[]
         for i in range(len(d)):
            if d[i][3]==clist[2] and int(d[i][4]) in klist:
                  b2.append(d[i])
         f2.write(b2[-1][0])
         f2.write(",")
         f2.write("sample")
         f2.write(",")
         f2.write(str(len(b2[-1][1])))
         f2.write(",")
         f2.write(b2[-1][2]+":"+str(clist[2]))
         f2.write("\n")
    nameFlag=0
    id=[]
    time1=[]
     ...
     while True:
        ...
        if(nameFlag==0):
            ...
            for i in range(len(b2)):
                ...
                if new[0][0]=="0":
                    newstr=new[0][1]
                    if b2[i][0]==entries[0] and int(newstr) in klist:
                     if int(new[3])==int(clist[2]):
                             print(1)
                             id.append(entries[0])
                             time1.append(entries[3])
                             number.append(entries[2])
                else:
                    newstr=new[0]
                    if b2[i][0]==entries[0] and int(newstr) in klist:
                        if int(new[3])==int(clist[2]):
    ...
    #Sorting the time based on the number of ordered items 
    zippedpairs=zip(number,time1)
    list_zippedpairs=list(zippedpairs)
    list_zippedpairs.sort()
    sortedTime=[list_zippedpairs[i][1] for i in range(len(list_zippedpairs))]
    ...
    list_index = [time1.index(i) for i in sortedTime]
    ...
    for i in sortedNumber:
     if(int(i)<=3):
        priority+=1
        ind=list_index[j]
        priorityList[ind]=priority
        j+=1

    for i in priorityList:
     if(i=='0'):
        priority+=1
        ind=priorityList.index(i)
        #print(ind)
        priorityList[ind]=priority
        
    print(priorityList)
    token_name=dict(zip(id,priorityList))
...
def table(a,b,datawin):
     ...
     if b.get()=="Breakfast":
        ...
        for i in range(len(dlist)):
            tot=0
            if int(dlist[i][4]) in breakfast1 and dlist[i][3]==k1[0]:
                count2=count2+1
                for j in range(len(dlist[i][1])):
                    for k in range(len(breakfast)):
                        if dlist[i][1][j][0]==breakfast[k][0]:
                            tot=tot+int(dlist[i][1][j][1])*breakfast[k][2]
                br=[count2,dlist[i][0],len(dlist[i][1]),tot*1.05]
                biglist.append(br)
     if b.get()=="Lunch":
        count2=0
        print(dlist)
        for i in range(len(dlist)):
            tot=0
            if int(dlist[i][4]) in lunch1 and dlist[i][3]==k1[0]:
                count2+=1
                for j in range(len(dlist[i][1])):
                    for k in range(len(lunch)):
                        if dlist[i][1][j][0]==lunch[k][0]:
                             tot=tot+int(dlist[i][1][j][1])*lunch[k][2]
                br=[count2,dlist[i][0],len(dlist[i][1]),tot*1.05]
                biglist.append(br)
     print(biglist)
     if b.get()=="Dinner":
        ...
        for i in range(len(dlist)):
            tot=0
            if int(dlist[i][4]) in dinner1 and dlist[i][3]==k1[0]:
                count2+=1
                for j in range(len(dlist[i][1])):
                    for k in range(len(dinner)):
                        if dlist[i][1][j][0]==dinner[k][0]:
                            tot=tot+int(dlist[i][1][j][1])*dinner[k][2]
                br=[1,dlist[i][0],len(dlist[i][1]),tot*1.05]
                biglist.append(br)
     if len(biglist)==1:
